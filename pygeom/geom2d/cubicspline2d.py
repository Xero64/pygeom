...
class CubicSpline2D():
    ...
    def validate(self) -> None:
        ...
        if isinstance(self.bctype, str):
            ...
            if not isinstance(self.bctype[0][1], Vector2D) or not isinstance(self.bctype[1][1], Vector2D):
                raise ValueError('Input bctype tuple elements must be Vector2D objects.')
        else:
    ...
    def d2r(self) -> Vector2D:
        if self._d2r is None:
            self._d2r = self.points.rmatmul(self.gmat)
        return self._d2r
