...
x = linspace(xa_val, xc_val, 101)

y = (a0_val + a1_val*x + a2_val*x**2)*ya_val + \
    (b0_val + b1_val*x + b2_val*x**2)*yb_val + \
    (c0_val + c1_val*x + c2_val*x**2)*yc_val

fig0 = figure(figsize=(12, 8))
ax0 = fig0.gca()
ax0.grid(True)
ax0.plot(x, y, label='Quadratic Interpolation')
_ = ax0.legend()

dy = (a1_val + 2*a2_val*x)*ya_val + \
     (b1_val + 2*b2_val*x)*yb_val + \
     (c1_val + 2*c2_val*x)*yc_val

fig1 = figure(figsize=(12, 8))
ax1 = fig1.gca()
ax1.grid(True)
ax1.plot(x, dy, label='Derivative of Quadratic Interpolation')
_ = ax1.legend()

d2y = (2*a2_val*ones(x.shape))*ya_val + \
      (2*b2_val*ones(x.shape))*yb_val + \
      (2*c2_val*ones(x.shape))*yc_val

fig2 = figure(figsize=(12, 8))
ax2 = fig2.gca()
ax2.grid(True)
ax2.plot(x, d2y, label='Second Derivative of Quadratic Interpolation')
_ = ax2.legend()
